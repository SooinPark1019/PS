...
signed main(){
    ...
    cin >> S;
    int a=5;
    for(int i=0; i<S.length()-1; i++) a*=10;
    vector<int> V;
    for(int i=0; i<10; i++){
        V.push_back(i);
    }
    ...
    do{
        ...
        //cout << temp << " " << mindis << " " << abs(a-abs(stoll(temp)-stoll(S))) << endl;
        if(mindis>abs(a-abs(stoll(temp)-stoll(S)))){
            mindis=abs(a-abs(stoll(temp)-stoll(S)));
            ans=V;
        }
