...
struct stHash {
    size_t operator()(const st& x) const {
        return hash<int>()(x.a) ^ hash<int>()(x.b) ^ hash<int>()(x.c) ^ hash<int>()(x.d);
    }
};
...
signed main(){
    ...
    mt19937 rng(15324896);
    uniform_int_distribution<int> dist(LLONG_MIN, LLONG_MAX);

    unordered_map<st, int, stHash> M;
    unordered_map<int, st> S;
    ...
    //cout << S[10].a << " " << S[10].b << " " << S[10].c << " " << S[10].d << "\n";
    ...
    if(type=="start"){
        ...
        for(int i=0; i<N; i++){
            int a;
            cin >> a;
            int nxtstatea, nxtstateb, nxtstatec, nxtstated;
            st A=S[a];
            nxtstatea=A.a;
            nxtstateb=A.b;
            nxtstatec=A.c;
            ...
            statec^=nxtstatec;
            stated^=nxtstated;
            if(M.find({statea, stateb, statec, stated})!=M.end()){
                cout << M[{statea, stateb, statec, stated}] << "\n";
            }
            else cout << -1 << "\n";
            //cout << statea << " " << stateb << " " << statec << " " << stated << "\n";
        }
        cout << statea << " " << stateb << " " << statec << " " << stated << "\n";
    }
    else{
        int statea, stateb, statec, stated;
        cin >> statea >> stateb >> statec >> stated;
        int N;
        cin >> N;
        for(int i=0; i<N; i++){
            int a;
