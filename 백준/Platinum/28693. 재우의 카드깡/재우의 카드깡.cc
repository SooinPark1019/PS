...
signed main(){
    ...
    for(int i=2; i<=2*N; i+=2){
        for(int j=i/2; j>=0; j--){
            int known=j*fastmul(i-j, mod-2)%mod;
            if(j>0) DP[i][j]+=known*(DP[i-2][j-1]+1)%mod;
            if(i>2*j){
                int unknown=(1-known+mod)%mod;
                int unknownagain=fastmul(i-j-1, mod-2);
                int unknownafterknown=j*fastmul(i-j-1, mod-2)%mod;
                int unknownafterunkown=(i-2*j-2)*fastmul(i-j-1, mod-2)%mod;
                DP[i][j]+=unknown*unknownagain%mod*(DP[i-2][j]+1)%mod;
                DP[i][j]+=unknown*unknownafterknown%mod*(DP[i-2][j]+2)%mod;
                DP[i][j]+=unknown*unknownafterunkown%mod*(DP[i][j+2]+1)%mod;
            }
        }
    }
