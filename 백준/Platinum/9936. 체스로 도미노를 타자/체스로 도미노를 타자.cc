    ...
    for(int i=0; i<3*N; i++){
        for(int j=0; j<=K; j++){
            for(int k=0; k<8; k++){
                ...
                if(j<K&&k%2==0){
                    if(i%3<2&&k%4==0){
                        DP[i+1][j+1][(k>>1)+1]=max(DP[i+1][j+1][(k>>1)+1], DP[i][j][k]+arr[i/3][i%3]+arr[i/3][i%3+1]);
                    }
                    if(i/3<N-1){
                        DP[i+1][j+1][(k>>1)+4]=max(DP[i+1][j+1][(k>>1)+4], DP[i][j][k]+arr[i/3][i%3]+arr[i/3+1][i%3]);
                    }
                }
            }
        }
