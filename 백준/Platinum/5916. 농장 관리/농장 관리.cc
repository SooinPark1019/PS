...
/*
먼저 루트가 고정되어 있는 문제부터 풀어보자
이진 트라이로 해결할 수 있구나

그렇다면 루트가 고정되지 않은 문제는?
하나를 루트로 잡
*/
...
struct Seg{
    ...
    void LazyUpdate(int node, int start, int end){
        if(lazy[node].first!=1||lazy[node].second!=0){
            seg[node]=(lazy[node].first*seg[node]%mod+(ll)(end-start+1)*lazy[node].second%mod)%mod;
            if(start!=end){
    ...
    void UpdateSeg(int node, int start, int end, int left, int right, ll value1, ll value2){
...
void hldplusupd(int a, int b, int val){
    ...
    seg.UpdateSeg(1, 0, N, in[a]+1, in[b], 1, val);
    return;
}
...
ll hldquery(int a, int b){
    ...
    if(dep[a]>dep[b]) swap(a, b);
    ret+=seg.SumSeg(1, 0, N, in[a]+1, in[b]);
    ret%=mod;
    return ret;
}
